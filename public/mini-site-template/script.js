...
const CustomNode = ({ data }) => {
  ...
  return React.createElement(
    ...
    React.createElement(ReactFlow.Handle, { type: 'target', position: ReactFlow.Position.Bottom, className: 'opacity-0' }),
    React.createElement(ReactFlow.Handle, { type: 'source', position: ReactFlow.Position.Bottom, className: 'opacity-0' }),
    // Apply the click handler and custom class to the content div instead
    React.createElement('div', { className: 'node-content', onClick: handleClick }, 
    ...
    showTooltip && data.description && React.createElement(
      'div',
...
const MindMapViewer = () => {
  ...
  try {
    const rawData = JSON.parse(dataScript.textContent.replace('/* MAP_DATA_PLACEHOLDER */', '{}'));
    ...
    return React.createElement('div', { style: { padding: '20px', color: 'red' } }, 'Error loading mind map data. Check console.');
  }
