...
const CustomNode = ({ data, id }: NodeProps<CustomNodeData>) => {
  ...
  return (
    <div
      ...
      onClick={handleClick}
    >
      <Handle id="target" type="target" position={Position.Bottom} className="opacity-0" />
  ...
  const exportAsHTML = () => {
    ...
    const htmlContent = `
      ...
      <head>
          ...
          <script src="https://unpkg.com/reactflow@11.7.0/dist/index.min.js"></script>
      ...
      <body>
          <div id="root"></div>
          <script>
              const { ReactFlow, Background, Controls, useNodesState, useEdgesState, Handle, Position } = window.ReactFlow;
                  ...
                  return React.createElement('div', {
                      ...
                      React.createElement('div', {
                          ...
                          React.createElement('div', {
                              ...
                              showTooltip && data.description && React.createElement('div', {
                                  key: 'tooltip',
                                  className: 'absolute z-10 mt-2 p-2 bg-white rounded-md shadow-lg border border-gray-200 text-sm text-gray-600 max-w-xs'
                              }, data.description)
                          ])
                      ])
                  ]);
              ...
              function CustomEdge({ id, sourceX, sourceY, targetX, targetY, data }) {
                  ...
                  return React.createElement(React.Fragment, null, [
                      ...
                      React.createElement('path', {
                          key: 'edge-path',
                          id: id,
                          className: \`react-flow__edge-path \${data?.strength === 'weak' ? 'weak' : ''}\`,
                          d: edgePath,
                          strokeWidth: 2,
                          stroke: '#666'
                      }),
              ...
              function FlowApp() {
                  const [nodes, setNodes] = useNodesState(${JSON.stringify(flowData.nodes)});
                  const [edges, setEdges] = useEdgesState(${JSON.stringify(flowData.edges)});
